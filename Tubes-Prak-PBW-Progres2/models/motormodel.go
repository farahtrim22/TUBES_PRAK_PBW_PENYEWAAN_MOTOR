...
func (m *MotorModel) Find(id int64, motor *entities.Motor) error {
	return m.conn.QueryRow("select * from motor where id = ?", id).Scan(&motor.Id, &motor.Merek, &motor.Tipe, &motor.JenisMotor, &motor.TahunProduksi, &motor.Warna, &motor.Stok, &motor.Harga)

}

func (m *MotorModel) Update(motor entities.Motor) error {
	_, err := m.conn.Exec(
		"update motor set merek = ?, tipe = ?, jenis_motor = ?, tahun_produksi = ?, warna = ?, stok = ?, harga = ? where id = ?",
...
func (m *MotorModel) Delete(id int64) {
	m.conn.Exec("delete from motor where id = ?", id)
}
